...
import { parseSource as parseSourceActual } from "./ts-parsing";

const parseSource = (str: string) => {
  const sourceFile = ts.createSourceFile("file.ts", str, ts.ScriptTarget.Latest, true);
  return parseSourceActual(ts, sourceFile);
};
